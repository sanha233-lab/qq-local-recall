...
- 视频防撤回：未播放视频被对方撤回后，界面不再显示"0%"加载圈，改为正常显示缩略图占位。根因：恢复逻辑保留了原始 `fileSize`（非零）和指向不存在文件的 `filePath`，QQNT 误判为"下载中"并渲染进度圈；修复后对无本地视频体的恢复消息将 `filePath` 和 `fileSize` 清零，重启后从 store 恢复的已下载视频本体不受影响（以 `fs.existsSync` 守卫区分）。
