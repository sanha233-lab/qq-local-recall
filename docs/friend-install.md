...
- 好友和群聊分别保存撤回记录。
- 普通图片和图片表情优先使用本地副本；缺失时默认通过当前 QQ 会话受限回源，可在“管理记录”关闭。语音、视频、文件、转发记录和复杂卡片保持原行为。
- 默认不拦截自己撤回的消息。
- 所有记录只保存在本机；媒体回源只访问 QQ 媒体服务，临时地址和凭据不写入记录或日志。
...
脚本会检查常见安装目录和 Windows QQ 卸载注册表，只接受版本为 `9.9.33-52230` 的目录，并校验该版本的官方 ASAR 入口。
...
当前版本恢复文字、QQ 原生表情、回复、普通图片、图片/动画表情和商城表情；图片依次使用有效本地文件、当前 QQ 会话中的完整临时媒体地址和已加载完成的页面快照。三者都不可用时，有限重试后显示“图片暂不可用”。语音、视频、文件、转发记录和复杂卡片保持原行为。
...
新版本 QQ 不会自动修改。重新运行安装前先执行 `-DryRun`；如果版本不再是 `9.9.33-52230`，请不要强行安装，等待兼容检查。
