...
- 媒体根目录是当前记录根目录下的 `media/`，本机当前对应 `<本机记录根>\media`。
...
- [ ] **Step 3: 部署前完整性记录与备份部署**
  ...
  记录 `<本机记录根>\records` 文件名清单、QQ.exe/application.asar SHA-256 和进程状态；完全退出 QQ 后运行：
...
- [ ] **Step 5: 最终实机验收**
  ...
  - 完全退出 QQ，确认 `<本机记录根>\media` 已生成按 SHA-256 命名的 GIF/WebP/PNG/JPEG 文件。
