...
test('renderer uses message-row native notices without detached toast or bubble badge', async () => {
  ...
  assert.match(source, /removeOrphanRecallNotices\(document\)/);
  ...
  assert.match(source, /findMessageContent\(document, String\(messageId\)\)/);
  assert.match(source, /captureRenderedMedia/);
