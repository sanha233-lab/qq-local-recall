...
1.4.0 正式交付只提供基础插件 ZIP、源码 ZIP、安装/回滚脚本、`vendor` 和 `SHA256SUMS.txt`，不生成朋友分发包；仓库内旧朋友包仅作为历史版本保留。
...
1.4.0 默认开启“缺失媒体自动回源”。本地图片不可用时，插件只通过当前 QQ 登录会话请求消息中已有的完整 QQ 媒体临时地址；关闭管理窗口中的开关后立即停止新网络请求，已保存的本地媒体仍可使用。设置保存在默认插件数据目录的 `settings.json`，不随记录目录迁移。
